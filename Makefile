...
kairos-image: ## Build Kairos image with challenger
	...
	docker build -f Dockerfile.kairos-image -t $(KAIROS_IMAGE_TAG) \
...
kairos-iso-uki: kairos-image ## Build UKI (Unified Kernel Image) ISO with secure boot signing
	...
	@if [ ! -d "$(KEYS_DIR)" ] || [ ! -f "$(KEYS_DIR)/db.key" ]; then \
		echo "Warning: Keys directory not found or missing keys at $(KEYS_DIR)"; \
		echo "Building without secure boot keys (UKI will not be signed)"; \
		docker run --rm \
			-v /var/run/docker.sock:/var/run/docker.sock \
